...
export const SCRIPT_SOURCE_META = {
  custom: { icon: '✍️', label: 'My script', title: 'Your pasted script, spoken word-for-word (no AI)' },
  ai: { icon: '🤖', label: 'AI retelling', title: 'Rewritten by AI in the style shown' },
  original: { icon: '📖', label: 'Original text', title: 'The untouched book text — no translation' },
};

export function sourceBadge(audio) {
  const meta = SCRIPT_SOURCE_META[audio?.scriptSource];
  if (!meta) return '';
  return `<span class="chapter-badge badge-src-${audio.scriptSource}" title="${meta.title}">${meta.icon} ${meta.label}</span>`;
}
...
export function versionLabel(a) {
  ...
  if (a.scriptSource === 'ai' && a.language !== 'en') parts.push(styleName(a.translationStyle));
...
export function audioBadges(a) {
  ...
  const styleBadge = (a.scriptSource === 'ai' && isHi)
    ? `<span class="chapter-badge" title="Storytelling style">🎭 ${styleName(a.translationStyle)}</span>`
    : '';
