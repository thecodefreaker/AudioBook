...
export function renderHome(books = [], activeJobs = {}) {
  ...
  if (!hasBooks) {
    if (empty) {
      ...
      empty.innerHTML = `
        <div class="empty">
          <div class="empty-icon" aria-hidden="true">Shelf</div>
  ...
  if (current && cont) {
    ...
    const chapterIdx = getLastRead(current.id);
     const where = chapterIdx != null ? `Last opened · Chapter ${chapterIdx + 1}` : 'No reading position yet';
    const p = pct(current);
    const dur = current.totalDuration ? formatTime(current.totalDuration) : null;
     const readyCount = current.chaptersWithAudio || 0;
     const totalCount = current.totalChapters || 0;
     const readyLabel = totalCount ? `${readyCount} of ${totalCount} chapters ready` : 'No audio yet';
        ...
        <div class="continue-card" data-home-act="open" data-book-id="${current.id}" role="button" tabindex="0">
          ...
          <div class="continue-card-main">
            ...
            <div class="continue-body">
              ...
              <p class="continue-sub">${escapeHtml(current.author || 'Unknown author')}</p>
               <p class="continue-where">${escapeHtml(where)} · ${escapeHtml(readyLabel)}</p>
          ...
          <div class="continue-actions">
            <button class="btn btn-primary" data-home-act="resume" data-book-id="${current.id}">
              ${ICON_PLAY} ${chapterIdx != null ? 'Resume reading' : 'Start reading'}
            </button>
          </div>
        </div>
        <div class="continue-card continue-listen-card" data-home-act="open" data-book-id="${current.id}" role="button" tabindex="0">
          <div class="continue-kicker">CONTINUE LISTENING</div>
          <div class="continue-listen-copy">
            <span class="listen-orb" aria-hidden="true">${ICON_PLAY}</span>
            <div>
              <h3 class="continue-title">${dur ? 'Your audiobook is ready' : 'Build your listening queue'}</h3>
              <p class="continue-sub">${dur ? `${dur} of audio available` : 'Choose a chapter to generate audio'}</p>
            </div>
          </div>
          <div class="continue-listen-stat">
            <span class="listen-stat-value">${p}%</span>
            <span class="listen-stat-label">audio ready</span>
          </div>
          <div class="continue-actions">
            <button class="btn btn-secondary" data-home-act="open" data-book-id="${current.id}">
              ${dur ? ICON_PLAY + ' Open player' : 'View chapters'}
            </button>
          </div>
        </div>
      </div>`;
