...
function actionsHtml(ch, versions) {
  ...
  if (gen) {
    return `
      <div class="ch-progress" role="status">
        <span>${escapeHtml(gen.message || 'Converting')}</span>
        <div class="progress"><div class="progress-fill" style="width:${gen.percent || 0}%"></div></div>
      </div>
      <div class="ch-actions">
        ...
        <button class="btn btn-ghost btn-sm" data-act="cancel" data-ch="${ch.chapterIndex}">Stop</button>
      </div>`;
  }
  ...
  if (!versions.length) {
    return `
      ...
      <div class="ch-actions">
        <div class="btn-split">
          <button class="btn btn-secondary btn-sm btn-split-main" data-act="convert" data-ch="${ch.chapterIndex}" title="Convert with the current settings">${ICON_PLAY} Convert</button>
          <button class="btn btn-secondary btn-sm btn-split-side" data-act="convert-options" data-ch="${ch.chapterIndex}" title="Convert with different settings…" aria-label="Convert with different settings">${ICON_SLIDERS}</button>
        </div>
        <button class="btn btn-ghost btn-icon btn-sm on-hover" data-act="preview" data-ch="${ch.chapterIndex}" title="Read chapter" aria-label="Read chapter">${ICON_EYE}</button>
      </div>`;
  }
  ...
  const badges = versions.length === 1
    ...
    : `<div class="ch-badges">
         ...
         <button class="chapter-badge" data-act="versions" data-ch="${ch.chapterIndex}" title="Manage all versions">📁 ${versions.length}</button>
       </div>`;

  return `
    ${badges}
    <div class="ch-actions">
      <button class="btn btn-primary btn-sm" data-act="play" data-ch="${ch.chapterIndex}" data-audio-index="${newestIdx}">${ICON_PLAY} Play</button>
      <div class="btn-split on-hover">
        <button class="btn btn-ghost btn-icon btn-sm btn-split-main" data-act="convert" data-ch="${ch.chapterIndex}" title="Convert again with the current settings" aria-label="Convert again">${ICON_REDO}</button>
        <button class="btn btn-ghost btn-icon btn-sm btn-split-side" data-act="convert-options" data-ch="${ch.chapterIndex}" title="Convert again with different settings…" aria-label="Convert with different settings">${ICON_SLIDERS}</button>
      </div>
      <button class="btn btn-ghost btn-icon btn-sm on-hover" data-act="preview" data-ch="${ch.chapterIndex}" title="Read chapter" aria-label="Read chapter">${ICON_EYE}</button>
 ...
 */
export function updateChapterRowProgress(chapterIdx, percent, message) {
  const row = document.querySelector(`.ch-row[data-ch="${chapterIdx}"]`);
  if (!row) return;

  const bar = row.querySelector('.ch-progress .progress-fill');
  const label = row.querySelector('.ch-progress span');
...
export function initChapterList(actions) {
  ...
  document.querySelector('#chapter-search-input')?.addEventListener('input', rerenderFromTop);
