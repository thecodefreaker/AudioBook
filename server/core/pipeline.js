...
async function processChapter({ job, chapter, log, control, emit, tracker }) {
  ...
  const script = await resolveScript({
    ...
    onProgress: (p) => {
      stage('script', 8 + Math.round(p.percent * 0.42), `Retelling chunk ${p.current}/${p.total}…`, {
  ...
  const ttsChunks = chunkText(spoken, config.ttsChunkMaxChars);
  log.info(
  ...
  const synth = await synthesizeChunked(ttsChunks, job.voice_id, outputPath, (p) => {
    checkCancel();
    stage('synth', 52 + Math.round(p.percent * 0.35), `Synthesising ${p.current}/${p.total}…`, {
      chunkCurrent: p.current, chunkTotal: p.total,
    });
    log.debug(`TTS chunk ${p.current}/${p.total} done`, { chapterIndex: idx, stage: 'synth' });
  });
  ...
  emit('chapter:complete', {
    ...
    scriptId: script.id, filePath: synth.filePath,
  });

  return { audioId, filePath: synth.filePath, duration };
}
...
export async function runJob(jobId, control) {
  ...
  const workers = Math.max(1, Math.min(Number(settings.chapterConcurrency) || 3, 8, total));
  if (workers > 1) {
