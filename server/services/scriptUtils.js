...
/**
 * Estimate the tokens Groq will bill for `text`.
 *
 * This is a PRE-FLIGHT estimate and nothing more. The real figure arrives in
 * `usage.total_tokens` on the response and the reservation is refunded to that
 * exact value immediately (see `ModelRateLimiter.observe`). The estimate exists
 * only because the TPM budget must be reserved BEFORE the request is sent —
 * reserving after the fact would mean discovering a breach by causing one,
 * which is what produced the old "Requested 9459, limit 6000" 413s.
 *
 * CALIBRATED against real billed tokens on 2026-08-11
 * (scratch/probe-4-estimator.mjs, 5 real chapters, ~8.5k chars each):
 *
 *   Latin       measured 3.83-3.97 chars/token  -> divisor 3.7 (≈5% conservative)
 *   Devanagari  measured 1.92 chars/token       -> divisor 1.8 (≈6% conservative)
 *
 * The previous divisors (3.2 and 1.4) over-counted input by a consistent 1.24x.
 * Because the reservation ceiling is `input + input*outputRatio`, that error
 * inflated BOTH terms and was the single largest remaining cause of
 * self-throttling once the per-model output ratios were fixed.
 *
 * Still rounds UP, and the divisors stay slightly below the measured values, so
 * this remains conservative — just honestly conservative rather than 24% off.
 */
export function estimateTokens(text) {
  ...
  return Math.ceil(rest / 3.7 + dev / 1.8);
}
