...
export async function buildAudiobook(bookId, { language, preferAudioIds } = {}) {
  ...
  const missing = chosen.filter((a) => !fs.existsSync(a.file_path));
  if (missing.length) {
    throw new Error(
  ...
  const merged = await mergeAudioFiles(chosen.map((a) => a.file_path), outPath);
  ...
  if (previous) {
    if (previous.file_path !== outPath) {
      try { fs.rmSync(previous.file_path, { force: true }); } catch { /* already gone */ }
    }
