...
router.get('/:audioId/stream', (req, res) => {
  try {
    ...
    const filePath = path.resolve(config.audioDir, audio.file_path);
...
router.get('/:audioId/vtt', (req, res) => {
  try {
    ...
    const vttPath = path.resolve(config.audioDir, audio.file_path).replace('.mp3', '.vtt');
...
router.get('/:audioId', (req, res) => {
  ...
  res.json({
    ...
    fileExists: fs.existsSync(path.resolve(config.audioDir, audio.file_path)),
  });
});
...
router.delete('/:audioId', (req, res) => {
  ...
  for (const p of [audio.file_path, audio.file_path?.replace(/\.mp3$/, '.vtt')]) {
    if (p) {
      const fullPath = path.resolve(config.audioDir, p);
      if (fs.existsSync(fullPath)) {
        try { fs.unlinkSync(fullPath); } catch (err) { logger.warn('Could not delete file', { p: fullPath, error: err.message }); }
      }
    }
  }
