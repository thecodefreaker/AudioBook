...
router.get('/:id/chapters/:idx/content', (req, res) => {
  ...
  const antigravityScript = scripts.find((s) => s.source === 'antigravity') || null;
  ...
  // `aiContent` / `customContent` / `antigravityContent` instead.
  const preferred = customScript || antigravityScript || aiScript;
  ...
  res.json({
    ...
    antigravityContent: antigravityScript?.content || '',
    antigravityScript: antigravityScript ? S.scriptMeta(antigravityScript) : null,
    customContent: customScript?.content || '',
    customScript: customScript ? S.scriptMeta(customScript) : null,
    /** @deprecated use aiContent / customContent / antigravityContent */
    translatedContent: preferred?.content || '',
    activeScript: preferred ? S.scriptMeta(preferred) : null,
    hasCustomScript: !!customScript,
    hasAiScript: !!aiScript,
    hasAntigravityScript: !!antigravityScript,
    latestAiScriptId: aiScript ? aiScript.id : null,
...
router.put('/:id/chapters/:idx/script', (req, res) => {
  ...
  const isNewVersion = req.body?.newVersion !== false && req.body?.overwrite !== true;
...
router.post('/:id/convert', wrap(async (req, res) => {
  ...
  const scriptSource = req.body?.scriptSource || settings.defaultScriptSource;
  if (!['ai', 'custom', 'original', 'antigravity'].includes(scriptSource)) {
  ...
  // ---- Prevent Race Conditions and Overlapping Jobs (RACE-01) -------------
  const activeJobs = db.getActiveJobs().filter((j) => j.book_id === book.id);
  const overlappingChapters = selectedChapters.filter((idx) =>
    activeJobs.some((job) => job.selected_chapters.includes(idx))
  );

  if (overlappingChapters.length > 0) {
    return refuse(res, book.id, 409, 'Duplicate request',
      `chapter${overlappingChapters.length > 1 ? 's' : ''} ${overlappingChapters.map(i => i + 1).join(', ')} ` +
      `${overlappingChapters.length > 1 ? 'are' : 'is'} already queued or processing.`
    );
  }
  ...
  db.createJob({
    ...
    groqModel: req.body?.groqModel !== undefined ? (req.body.groqModel || null) : (settings.groqModel || null),
    translationStyle,
    scriptSource,
...
router.post('/:id/jobs/:jobId/retry', (req, res) => {
  ...
  // Retry used to skip the duplicate guard that `/convert` enforces, so
  // double-clicking Retry — or retrying while the original job was still
  // running — enqueued a second job for the same chapter and produced two
  // audio rows for it.
  const activeJobs = db.getActiveJobs().filter((j) => j.book_id === previous.book_id);
  const overlapping = targets.filter((idx) =>
    activeJobs.some((job) => job.selected_chapters.includes(idx))
  );
  if (overlapping.length) {
    return res.status(409).json({
      error: 'Duplicate request',
      message: `Chapter${overlapping.length > 1 ? 's' : ''} ${overlapping.map((i) => i + 1).join(', ')} ` +
        `${overlapping.length > 1 ? 'are' : 'is'} already queued or processing.`,
    });
  }

  // Decide per chapter whether an existing script can be reused, then GROUP by
  // the resulting action. The old loop created one job per chapter, so
  // retrying 100 failures enqueued 100 jobs and flooded the queue — each one
  // paying full job overhead for a single chapter.
  const byAction = new Map(); // key -> { action, scriptId, chapters[] }

  for (const chIdx of targets) {
    const chapter = db.getChapterByBookAndIndex(previous.book_id, chIdx);
    let scriptId = previous.script_id || null;
    let action = previous.action || 'both';

    if (action === 'both' && !scriptId && chapter) {
      const scripts = db.getScriptsByChapterId(chapter.id);
      const match = scripts
        .filter((s) => s.source === previous.script_source && s.language === previous.language)
        .filter((s) => previous.script_source !== 'ai' || s.style === previous.translation_style)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];

      // If a matching script was created during or after the original job's run, reuse it.
      if (match && new Date(match.created_at) >= new Date(previous.created_at)) {
        scriptId = match.id;
        action = 'audio';
      }
    }

    // An `audio` run is pinned to ONE script id, so those cannot be grouped
    // with each other — each keeps its own job. Everything else groups.
    const key = action === 'audio' ? `audio:${scriptId}` : `${action}`;
    if (!byAction.has(key)) byAction.set(key, { action, scriptId, chapters: [] });
    byAction.get(key).chapters.push(chIdx);
  }
  ...
  for (const group of byAction.values()) {
    ...
    db.createJob({
      ...
      selectedChapters: group.chapters,
      ...
      mergeOutput: false,
      action: group.action,
      scriptId: group.action === 'audio' ? group.scriptId : null,
    });
  ...
  // If the previous job was stuck in 'interrupted', mark it as superseded
  // so the frontend stops prompting the user to resume it on every page load.
  if (previous.status === 'interrupted') {
    db.updateJob(previous.id, {
      status: 'failed',
      error_log: (previous.error_log ? previous.error_log + '\n' : '') + 'Superseded by resume request.'
    });
  }
  ...
  res.status(202).json({
    ...
    jobIds,
    queue: firstStatus,
    message: `Retrying ${targets.length} chapter(s) in ${jobIds.length} job(s).`,
  });
});
