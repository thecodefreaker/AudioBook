...
export function initDatabase() {
  ...
  addColumn('generation_jobs', 'custom_prompt TEXT');
  ...
  addColumn('chapter_scripts', 'custom_prompt TEXT');
  ...
  // ---------------------------------------------------------------
  // Self-heal: any job left "processing"/"queued"/"stopping" from a
  // previous process is orphaned — the in-memory queue no longer knows it.
  // Mark it interrupted so the UI can offer Resume (fixes B9).
  //
  // `stopping` must be listed: a process killed while a job was being
  // cancelled would otherwise be stranded in that state forever, showing a
  // permanently disabled Stop button and blocking the duplicate-request guard
  // for its chapters.
  // ---------------------------------------------------------------
  try {
    const res = db.prepare(
      `UPDATE generation_jobs SET status = 'interrupted', finished_at = CURRENT_TIMESTAMP
       WHERE status IN ('processing', 'queued', 'stopping')`
    ).run();
    if (res.changes > 0) {
...
export function createJob({
  ...
  translationStyle, scriptSource, customPrompt, speakingRate, useGlossary, allowGoogleFallback, mergeOutput,
  action, scriptId,
}) {
  const stmt = getDb().prepare(`
    INSERT INTO generation_jobs (
      id, book_id, selected_chapters, language, voice_id, tts_provider,
      groq_api_key, groq_model, translation_style, script_source, custom_prompt, speaking_rate, total_chapters,
  ...
  stmt.run(
    ...
    scriptSource || 'ai', customPrompt || null, speakingRate || 1.0, selectedChapters.length,
...
export function getActiveJobs() {
  // `stopping` MUST be included. It is still a live job: it owns chapters, it
  // is still in the queue's running map, and crash recovery must reclaim it.
  // Omitting it would let the duplicate-request guard in /convert wave through
  // a second job for chapters that are still being torn down, and would leave
  // a killed-mid-stop job stuck in `stopping` forever after a restart.
  const jobs = getDb()
    .prepare(`SELECT j.*, b.title AS book_title FROM generation_jobs j
              LEFT JOIN books b ON b.id = j.book_id
              WHERE j.status IN ('queued','processing','stopping') ORDER BY j.created_at ASC`)
    .all();
  return jobs.map((job) => ({
    ...job,
...
export function createScript({
  ...
  provider, model, promptVersion, content, spokenContent, customPrompt, tokenCount
}) {
  const stmt = getDb().prepare(`
    INSERT INTO chapter_scripts (
      id, book_id, chapter_id, source, language, script_kind, style,
      provider, model, prompt_version, content, spoken_content, custom_prompt, token_count, char_count
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  stmt.run(
    id, bookId, chapterId, source, language, scriptKind || null, style || null,
    provider || null, model || null, promptVersion || null,
    content, spokenContent || content, customPrompt || null, tokenCount || 0, (content || '').length
  );
...
/** Upsert the custom script — editing replaces only the most recent custom one. */
...
/** Add a brand new custom script version without overwriting previous versions. */
export function addCustomScript({ id, bookId, chapterId, content, scriptKind, spokenContent, language = 'hi' }) {
  return createScript({
    id: id || randomId(),
    bookId,
    chapterId,
...
export function getChapterRollup(bookId) {
  return getDb().prepare(`
    SELECT
      ...
      (SELECT COUNT(*) FROM chapter_scripts s
        WHERE s.chapter_id = c.id AND s.source = 'ai') AS hasAiScript,
      (SELECT id FROM chapter_scripts s
        WHERE s.chapter_id = c.id AND s.source = 'ai' ORDER BY created_at DESC LIMIT 1) AS latestAiScriptId,
      (SELECT COUNT(*) FROM chapter_scripts s
